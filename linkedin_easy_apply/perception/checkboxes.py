...
def detect_checkbox_groups(page):
    ...
    try:
        ...
        for i in range(checkbox_count):
            ...
            try:
                parent_info = checkbox.evaluate('''el => {
                    let current = el;
        ...
        for container_id, group_data in checkbox_groups.items():
            ...
            mutually_exclusive_starts = ['yes', 'no', 'not applicable', 'decline', 'i prefer not']
            ...
            is_radio_equivalent = (
                2 <= len(checkboxes_in_group) <= 4 and
                (any(set(labels) >= pattern for pattern in mutually_exclusive_patterns) or
                 has_exclusive_labels >= 2)
            )
