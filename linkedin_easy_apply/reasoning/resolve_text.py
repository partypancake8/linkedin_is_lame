...
def resolve_field_answer(
    ...
    Args:
        ...
        field_classification: Classification string
    ...
    if matched_key and matched_key in ANSWER_BANK:
        ...
        if field_classification == "NUMERIC_FIELD":
            ...
            if not answer.replace(".", "").isdigit():
                print(
                    f"  ⚠️ Warning: Numeric field matched to non-numeric answer '{matched_key}'"
                )
                return None
    ...
    if debug_unresolved and job_id and job_url:
        ...
        tier = "unknown"
        if field_classification.startswith("TIER1_"):
            tier = "tier-1"
        elif field_classification.startswith("TIER2_"):
            tier = "tier-2"
        elif field_classification.startswith("SKIP_"):
            tier = "skip"

        record_unresolved_field(
            job_id=job_id,
            job_url=job_url,
            state_at_exit="TEXT_UNRESOLVED",
            skip_reason="unresolved_field",
            field_type="text",
            question_text=field_metadata.get("label", ""),
            options=None,
            classification=field_classification,
            tier=tier,
            eligibility=(tier in ["tier-1", "tier-2"]),
            confidence="none",
            matched_key=None,
        )
