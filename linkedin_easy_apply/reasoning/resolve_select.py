...
from linkedin_easy_apply.reasoning.normalize import (
    normalize_text,
    normalize_option_text,
)


def resolve_select_answer(
    select_metadata, debug_unresolved=False, job_id=None, job_url=None
):
    """
    ...
    Args:
        select_metadata: Field metadata dictionary
        debug_unresolved: If True, record unresolved fields to debug log
        job_id: Job ID for debug logging
        job_url: Job URL for debug logging
    ...
    select_mappings = {
        ...
        (
            "select",
            "level",
        ): "language_proficiency",  # Common pattern: "Select your level"
    ...
    if matched_key and matched_key in ANSWER_BANK:
        ...
        # Handle start_date_notice_period - discrete time offsets only
            ...
            for i, opt_text in enumerate(option_texts):
                ...
                if expected_weeks in opt_normalized and (
                    "week" in opt_normalized or "month" in opt_normalized
                ):
                    return (i, "high", matched_key)
        ...
        # Handle education_enrollment_status - binary Yes/No only
            ...
            for i, opt_text in enumerate(option_texts):
                ...
                is_yes = opt_normalized == "yes" or any(
                    p in opt_normalized for p in yes_patterns if len(p) > 3
                )
                ...
                is_no = opt_normalized == "no" or any(
                    p in opt_normalized for p in no_patterns if len(p) > 3
                )
        ...
        # Handle language_proficiency - standard proficiency levels
            ...
            if expected_level in proficiency_keywords:
                for i, opt_text in enumerate(option_texts):
                    ...
                    # Skip placeholder options
                    is_placeholder = any(
                        p in opt_normalized for p in placeholder_patterns
                    )
                    if is_placeholder:
                        continue
    ...
    # Record for debug if enabled
    if debug_unresolved and job_id and job_url:
        from linkedin_easy_apply.debug.unresolved_collector import (
            record_unresolved_field,
        )

        record_unresolved_field(
            job_id=job_id,
            job_url=job_url,
            state_at_exit="SELECT_UNRESOLVED",
            skip_reason="unresolved_field",
            field_type="select",
            question_text=select_metadata.get("label", ""),
            options=select_metadata.get("option_texts", []),
            classification=matched_key if matched_key else "unmatched",
            tier="unknown",
            eligibility=False,
            confidence="low",
