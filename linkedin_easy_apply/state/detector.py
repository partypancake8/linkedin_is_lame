...
def detect_state(page, step_number):
    ...
    try:
        ...
        if modal_visible:
            ...
            has_submit = any(page.locator(f'[role="dialog"] {sel}').count() > 0 for sel in submit_selectors)
            has_next = any(page.locator(f'[role="dialog"] {sel}').count() > 0 for sel in next_selectors)
            has_review = any(page.locator(f'[role="dialog"] {sel}').count() > 0 for sel in review_selectors)
            ...
            if step_number > 1:  # Skip first step to reduce noise
                ...
                if detected_buttons:
                    print(f"  [State Detector] Buttons found: {', '.join(detected_buttons)}")
